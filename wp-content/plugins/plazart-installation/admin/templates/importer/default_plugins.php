...
?>
    <?php $uniqid  = uniqid();?>
    <div class="border-bottom border-gray mt-3 mb-4 ml-n3 mr-n3 pl-3 pr-4 pb-3 plzinst-plugin__install">
        ...
        <?php if($this -> item['plugins'] && count($this -> item['plugins'])){?>
            <div class="d-flex justify-content-between align-items-end w-100 pl-2 small text-muted item">
                <div class="w-100 flex-1 title">
                    <div class="custom-control custom-checkbox d-inline-block bg-white">
                        <input type="checkbox" class="custom-control-input" id="plzinst-checkbox__plugin-<?php
                        echo $uniqid;?>" data-checkbox-plugin-all>
                        <label class="position-relative custom-control-label" for="plzinst-checkbox__plugin-<?php echo $uniqid;?>"><?php echo __("All Plugins"); ?></label>
                    </div>
                </div>
                <span class="text-right plzinst-plugin__actions">
                    <a href="javascript:"
                       class="js-plzinst-plugin__install-all text-danger d-none"><?php
                        echo __('Install Selected', $this -> text_domain); ?></a>
                    <a href="javascript:"
                       class="js-plzinst-plugin__update-all text-info d-none"><?php
                        echo __('Update Selected', $this -> text_domain); ?></a>
                    <a href="javascript:"
                       class="js-plzinst-plugin__activate-all text-primary d-none"><?php
            ...
            <div class="items mh-px-300 pt-2 overflow-auto">
                ...
                foreach($this -> item['plugins'] as $plugin_code => $plugin){
                    ...
                    $installedVersion   = AdminFunctions::get_plugin_version_by_slug($plugin_slug);
                    ...
                    <div class="d-flex justify-content-between w-100 mb-2 pl-2 small text-muted item" data-plugin-item>
                        <div class="w-100 flex-1 title">
                            <div class=" custom-control custom-checkbox d-inline-block bg-white">
                                <input type="checkbox" class="custom-control-input" id="plzinst-checkbox__plugin-<?php
                                echo $plugin_slug; ?>"<?php echo (AdminFunctions::is_plugin_active($plugin_slug) && !$canUpdate)?' disabled':''; ?>>
                                <label class="position-relative custom-control-label" for="plzinst-checkbox__plugin-<?php
                                echo $plugin_slug; ?>"><?php echo $plugin['name']; ?></label>
                            </div>
                            <?php
                            if($canUpdate){
                            ?>
<!--                                    --><?php //if(AdminFunctions::is_plugin_active($plugin_slug)){ ?>
<!--                                    <span class="badge badge-success">--><?php //echo __('Activated', $this -> text_domain); ?><!--</span>-->
<!--                                    --><?php //}else{ ?>
<!---->
<!--                                    <span class="badge badge-info">--><?php //echo __('Not activated', $this -> text_domain); ?><!--</span>-->
<!--                                    --><?php //} ?>
                        ...
                        <span class="">
                            ...
                            if($canUpdate){
                                ...
                                if(AdminFunctions::is_plugin_active($plugin_slug)){
                                    $btnText    = 'Activated';
                                    $btnClass   = ' text-success activated';
                                }elseif(AdminFunctions::is_plugin_installed($plugin_slug)){
                                        $btnText    = 'Activate';
                            ...
                            ?>
                            <a href="javascript:"
                               class="js-plzinst-plugin__install<?php echo $btnClass;?>"
                               data-plugin="<?php echo esc_attr($plugin_slug); ?>"
                               data-nonce="<?php echo esc_attr(wp_create_nonce(PLAZART_INSTALLATION_NAME.'-action')); ?>"
