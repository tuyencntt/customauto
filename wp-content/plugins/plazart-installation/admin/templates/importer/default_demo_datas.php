...
if(isset($this -> item) && isset($this -> item['demo-datas']) && count($this -> item['demo-datas'])){
    ...
    <form id="plzinst-import__<?php echo $code; ?>" data-demo-id="<?php echo $code; ?>">
    <?php foreach($item['demo-datas'] as $i => $data){?>
        <div class="js-plzinst-demo-item custom-checkbox pt-2 pb-2 ml-n3 mr-n3 pl-5">
            <input type="checkbox" class="custom-control-input" id="plzinst-checkbox__<?php
...
//            echo isset($data['options'])?' data-demo-options="'.$data['options'].'"':'';
            ...
            <label class="custom-control-label" for="plzinst-checkbox__<?php echo $code;?>-<?php echo $data['slug'];?>">
